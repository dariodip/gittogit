package org.darsquared.gitprotocol;

public class Operationmessage {

    public final static String NO_REPO_FOUND = "No repo found";
    public final static String PUSH_SUCCESSFULL = "Push successfull";
    public final static String PULL_SUCCESSFULL = "Pull successfull";
    public final static String PULL_REQUIRED = "Pull required";
    public final static String SOMETHING_GONE_WRONG = "Something gone wrong";
    public final static String NO_FILE_CHANGED = "No file changed";



}
